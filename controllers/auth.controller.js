...
async function register(req, res) {
  try {
    ...
    const exists = await User.findOne({ $or: [{ email }, { employeeId }, { cnic }] });
...
async function login(req, res) {
  ...
  try {
    ...
    if (!user.emailVerified) return res.status(403).json({ message: 'Please verify your email via OTP first' });
    if (!user.isApproved) return res.status(403).json({ message: 'Account awaiting approval' });
...
async function getMe(req, res) {
  try {
    const user = await User.findById(req.user.id)
      .select('_id fullName profileImageUrl email employeeId department role isApproved emailVerified createdAt updatedAt')
...
module.exports = { register, verifyEmailOtp, resendEmailOtp, login, logout , getMe };
